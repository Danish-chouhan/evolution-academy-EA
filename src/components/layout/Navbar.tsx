          ...
          <div className="flex justify-between items-center h-[72px]">
            ...
            <div className="flex items-center gap-4 sm:gap-8">
              ...
              <Link href="/" className="flex items-center gap-1.5 sm:gap-2">
                <img src="/images/logo.png" alt="Evolution Academy Logo" className="w-6 sm:w-8 lg:w-8 xl:w-10 h-auto object-contain" />
                <img src="/images/name.jpeg" alt="Evolution Academy" className="h-3 sm:h-4 lg:h-4 xl:h-6 w-auto object-contain" />
              </Link>
              
              <CategoryDropdown />
            </div>

            <div className="hidden lg:flex items-center lg:space-x-3 xl:space-x-6 lg:text-xs xl:text-sm">
              <Link href="/batches" className="text-gray-600 hover:text-brand-purple font-semibold whitespace-nowrap">Batches</Link>
              <Link href="/results" className="text-gray-600 hover:text-brand-purple font-semibold whitespace-nowrap">Results</Link>
              <Link href="/store" className="text-gray-600 hover:text-brand-purple font-semibold whitespace-nowrap">Products</Link>
              <Link href="/about" className="text-gray-600 hover:text-brand-purple font-semibold whitespace-nowrap">About Us</Link>
              
              {/* More Dropdown */}
              <div className="relative group">
                <button className="text-gray-600 hover:text-brand-purple font-semibold flex items-center gap-1 py-4 whitespace-nowrap">
                  More <ChevronDown size={14} className="group-hover:rotate-180 transition-transform" />
                </button>
                <div className="absolute top-full left-0 w-52 bg-white border border-gray-100 shadow-xl rounded-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50 py-2 -mt-2">
                  {user ? (
                    <>
                      ...
                      <Link href="/free-library" className="block px-4 py-2.5 text-sm text-gray-700 hover:bg-purple-50 hover:text-brand-purple font-semibold transition-colors">Free Library</Link>
                      <Link href="/mentorship" className="block px-4 py-2.5 text-sm text-gray-700 hover:bg-purple-50 hover:text-brand-purple font-semibold transition-colors">Mentorship</Link>
                    </>
                  ) : (
                    <>
                      <Link href="/free-library" className="block px-4 py-2.5 text-sm text-gray-700 hover:bg-purple-50 hover:text-brand-purple font-semibold transition-colors">Free Library</Link>
            ...
            <div className="flex items-center gap-2 sm:gap-4 relative">
              ...
              <div className="hidden sm:flex items-center bg-gray-100 rounded-lg px-2 xl:px-3 py-1.5 xl:py-2 border border-gray-200 relative group">
                <Search size={16} className="text-gray-400" />
                <input 
                  type="text" 
                  placeholder="Search..." 
                  className="bg-transparent border-none outline-none text-xs xl:text-sm ml-1 xl:ml-2 w-16 lg:w-20 xl:w-32 placeholder-gray-400"
                />
              ...
              {user && (
                <div className="relative group">
                  <button 
                    ...
                    className="bg-brand-purple hover:bg-purple-700 text-white px-2 sm:px-3 xl:px-5 py-1.5 sm:py-2 xl:py-2.5 rounded-lg font-bold flex items-center gap-1 xl:gap-2 transition-colors text-[10px] sm:text-xs xl:text-sm shadow-sm"
                  >
                    {user.avatar ? (
                      <img src={user.avatar} alt="Profile" className="w-4 h-4 xl:w-5 xl:h-5 rounded-full object-cover hidden sm:block" />
                    ) : (
              ...
              {!user && (
                <Link href="/auth/login" className="bg-brand-purple hover:bg-brand-purple/90 text-white px-3 sm:px-4 xl:px-5 py-2 xl:py-2.5 rounded-lg font-bold flex items-center gap-1.5 xl:gap-2 transition-colors text-[11px] sm:text-xs xl:text-sm shadow-sm">
                  <User size={16} className="hidden sm:block" />
                  <span>Login</span>
                </Link>
              )}
        ...
        {isMobileMenuOpen && (
          <div className="lg:hidden absolute top-[72px] left-0 w-full bg-white border-b border-gray-200 shadow-xl overflow-hidden animate-in slide-in-from-top-2">
            <div className="flex flex-col px-4 pt-2 pb-6 space-y-1">
              ...
              <Link onClick={() => setIsMobileMenuOpen(false)} href="/batches" className="block py-2 px-4 rounded-xl text-gray-700 text-sm font-bold hover:bg-gray-50">Batches</Link>
              <Link onClick={() => setIsMobileMenuOpen(false)} href="/results" className="block py-2 px-4 rounded-xl text-gray-700 text-sm font-bold hover:bg-gray-50">Results</Link>
              <Link onClick={() => setIsMobileMenuOpen(false)} href="/store" className="block py-2 px-4 rounded-xl text-gray-700 text-sm font-bold hover:bg-gray-50">Products</Link>
              <Link onClick={() => setIsMobileMenuOpen(false)} href="/about" className="block py-2 px-4 rounded-xl text-gray-700 text-sm font-bold hover:bg-gray-50">About Us</Link>
              
              <button onClick={() => setIsMoreMobileOpen(!isMoreMobileOpen)} className="flex items-center justify-between w-full py-2 px-4 rounded-xl text-gray-700 text-sm font-bold hover:bg-gray-50">
                <span>More</span>
                ...
                <div className="pl-4 ml-4 border-l-2 border-gray-100 flex flex-col space-y-1 mb-2">
                  {user ? (
                    <>
                      ...
                      <Link onClick={() => setIsMobileMenuOpen(false)} href="/free-library" className="block py-2.5 px-4 rounded-xl text-gray-600 font-semibold hover:bg-gray-50 hover:text-brand-purple">Free Library</Link>
                      <Link onClick={() => setIsMobileMenuOpen(false)} href="/mentorship" className="block py-2.5 px-4 rounded-xl text-gray-600 font-semibold hover:bg-gray-50 hover:text-brand-purple">Mentorship</Link>
                    </>
                  ) : (
                    <>
                      <Link onClick={() => setIsMobileMenuOpen(false)} href="/free-library" className="block py-2.5 px-4 rounded-xl text-gray-600 font-semibold hover:bg-gray-50 hover:text-brand-purple">Free Library</Link>
